...
class WalkerCharacter {
    ...
    // 拖动只允许沿着 dock 顶部横向移动，避免角色跑到内容区中间挡住屏幕。
    func beginPointerTracking(at screenPoint: NSPoint) {
        pointerDownScreenPoint = screenPoint
        pointerDownWindowOrigin = window.frame.origin
        isDraggingCharacter = false
    }

    func continuePointerTracking(to screenPoint: NSPoint) {
        guard let startPoint = pointerDownScreenPoint,
              let startOrigin = pointerDownWindowOrigin else { return }

        let deltaX = screenPoint.x - startPoint.x
        let deltaY = screenPoint.y - startPoint.y
        if !isDraggingCharacter, hypot(deltaX, deltaY) < dragThreshold {
            return
        }
    ...
    func endPointerTracking(at screenPoint: NSPoint) -> Bool {
        defer { cancelPointerTracking() }

        guard pointerDownScreenPoint != nil else { return false }
        if isDraggingCharacter {
            continuePointerTracking(to: screenPoint)
            manualWindowOrigin = nil
            isDraggingCharacter = false
            pauseEndTime = CACurrentMediaTime() + Double.random(in: 1.0...2.0)
            return false
        }
    ...
    func cancelPointerTracking() {
        pointerDownScreenPoint = nil
        pointerDownWindowOrigin = nil
        isDraggingCharacter = false
    }
    ...
    private func screenForCharacterOrigin(_ origin: NSPoint) -> NSScreen? {
        let samplePoint = NSPoint(x: origin.x + displayWidth / 2, y: origin.y + displayHeight / 2)
        return NSScreen.screens.first(where: { $0.frame.contains(samplePoint) }) ?? window.screen ?? controller?.activeScreen ?? NSScreen.main
    }
    ...
    private func clampedCharacterOrigin(for origin: NSPoint) -> NSPoint {
        guard let screen = screenForCharacterOrigin(origin) else { return origin }
        let visibleFrame = screen.visibleFrame.insetBy(dx: 6, dy: 6)
        let maxX = visibleFrame.maxX - displayWidth
        return NSPoint(
            x: max(visibleFrame.minX, min(origin.x, maxX)),
            y: dockAnchoredY(for: screen)
        )
    }

    private func syncPositionProgressToCurrentWindow() {
        guard let screen = screenForCharacterOrigin(window.frame.origin) else { return }
        let usableWidth = max(screen.frame.width - displayWidth, 1)
        let relativeX = window.frame.origin.x - screen.frame.minX - currentFlipCompensation
